...
export const UafPrintView: React.FC<UafPrintViewProps> = ({ classes }) => {
  ...
  return (
    <div className="bg-white p-8 font-sans print:p-0 print:m-0" id="uaf-print-body">
      ...
      <table className="w-full border-collapse border-2 border-black text-[10px]">
        ...
        <tbody>
          {/* We'll just show Monday for the print demo as per PDF page 1 */}
          {allRooms.map((room, idx) => (
            <tr key={idx} className="h-16">
              ...
              {TIME_SLOTS.map((slot, sIdx) => {
                ...
                // Simple slot matching for print (this is a fixed grid for the PDF replication)
                const startHour = slot.split(':')[0];
                const session = classes.find(c => c.roomId === room.id && c.startTime.startsWith(startHour.padStart(2, '0')));
                const faculty = session ? FACULTY.find(f => f.id === session.facultyId) : null;
                  ...
                  <td key={sIdx} className="border border-black p-1 text-center relative">
                    {session ? (
                      <div className="flex flex-col justify-center h-full">
                        ...
                        <div className="text-[8px] italic">{faculty?.name}</div>
                      </div>
                    ) : (
